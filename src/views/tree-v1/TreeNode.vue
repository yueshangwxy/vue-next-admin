...
const isChecked = computed(() => props.selectedNodes.has(props.node.id))

const isIndeterminate = computed(() => {
  if (!hasChildren.value) return false
  const children = props.node.children
  const selectedCount = children.filter(child => props.selectedNodes.has(child.id)).length
  return selectedCount > 0 && selectedCount < children.length
})

function handleChange() {
  emit('toggleSelect', props.node, !isChecked.value)
}
